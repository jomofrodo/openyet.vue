...
DROP VIEW IF EXISTS cv_14day_trend;
CREATE VIEW cv_14day_trend AS
SELECT countrycode, statecode, county,
	date, 
	confirmed,
	confirmedincrease as confd,
	positive,
	positivieincrease as positived,
	death,
	deathincrease as deathd,
	confdp,confdpt,
	positivedp,positivedpt,
	deathdp, deathdpt
FROM  combined
WHERE date > now() - interval (14 'days');
	

DROP VIEW IF EXISTS cv_national_14day CASCADE;
CREATE VIEW cv_national_14day AS
...
DROP VIEW IF EXISTS cv_state_14day CASCADE;
CREATE VIEW cv_state_14day AS
SELECT
...
DROP VIEW IF EXISTS cv_county_14day CASCADE;
CREATE VIEW cv_county_14day AS
SELECT
	countrycode,statecode,county,
		date_trunc('week', date) AS week,
		max(confirmed) as confirmed,
